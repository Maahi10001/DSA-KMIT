...
Explanation:
------------
transfer the character 'e' in 4 steps, to get 'i'. 
...
import java.util.*;
class program{
    public static boolean helper(char c,char g,int len){
        for(int i=0;i<len;i++){
            //System.out.print(c);
            if(c=='z'){
                c='a';
    ...
    public static boolean check(String s1,String s2,int len){
        for(int i=0;i<s1.length();i++){
            if(s1.charAt(i)!=s2.charAt(i)){
                if(helper(s1.charAt(i),s2.charAt(i),len)){
                    continue;
                }
                else{
                    return false;
    ...
    public static void main(String []args){
        ...
        String s2=sc.next();
        int len=sc.nextInt();
        System.out.print(check(s1,s2,len));
    }
}