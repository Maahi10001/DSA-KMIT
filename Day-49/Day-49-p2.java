...
class program
{
    public static int pathcost(int arr[][],int i, int j,int r,int c,int dp[][]){
        if(i<0 ||i==r||j==c)
            return 0;
        if(dp[i][j]!=0)
            return dp[i][j];
        int rightup=pathcost(arr,i-1,j+1,r,c,dp);
        int right=pathcost(arr,i,j+1,r,c,dp);
        int rightdown=pathcost(arr,i+1,j+1,r,c,dp);
        
        return dp[i][j]=arr[i][j]+Math.max(Math.max(rightup,right),rightdown);
    } 
    public static int helper(int arr[][],int r,int c){
       ...
       for(int i=0;i<r;i++)
       for(int j=0;j<c;j++){
           int pc=pathcost(arr,i,j,r,c,dp);
           max=Math.max(pc,max);
       }
        return max;
    }
